 ...
 * @since 2.0.0
 */
fun IManagedEntity.getDataFile(context: SchemaContext, partitionId:Int = partitionId(context), descriptor: EntityDescriptor = descriptor(context)) = context.getPartitionDataFile(descriptor, partitionId)
 ...
 * @since 2.0.0
 */
fun IManagedEntity.records(context: SchemaContext, partitionId: Int = partitionId(context), descriptor: EntityDescriptor = descriptor(context)):DiskMap<Any, IManagedEntity> = getDataFile(context, partitionId, descriptor).getHashMap(descriptor.identifier!!.type, descriptor.entityClass.name)
