...
class BPETokenizer(
     ...
     */
    fun extractUniqueTokens(text: String): Set<String> {
        val uniqueTokens = mutableSetOf<String>()
        ...
        try {
            ...
            for (match in tokenPattern.findAll(normalizedText)) {
                ...
                if (start > lastEnd) {
                    val unknownText = normalizedText.substring(lastEnd until start)
                    unknownText.forEach { uniqueTokens.add(it.toString()) }
                }

                val groupValues = match.groupValues
                when {
                    groupValues[1].isNotEmpty() -> {} // Whitespace: ignore
                    groupValues[2].isNotEmpty() -> uniqueTokens.add(groupValues[2]) // Keyword
                    groupValues[3].isNotEmpty() -> uniqueTokens.add(groupValues[3]) // Operator
                    groupValues[4].isNotEmpty() -> uniqueTokens.add(groupValues[4]) // Identifier
                    groupValues[5].isNotEmpty() -> uniqueTokens.add(groupValues[5]) // Number
                    groupValues[6].isNotEmpty() -> uniqueTokens.add(groupValues[6]) // Punctuation
                    groupValues[7].isNotEmpty() -> uniqueTokens.add("<|math_${groupValues[7]}|>") // Math symbol
                    groupValues[8].isNotEmpty() -> {
                        val fractionMatch = Regex("(\\d+)/(\\d+)").matchEntire(groupValues[8])
                        if (fractionMatch != null) {
                            val (num, den) = fractionMatch.destructured
                            uniqueTokens.add("<|math_frac_${num}_${den}|>")
                        } else {
                            uniqueTokens.add(groupValues[8])
                        }
                    }
                }
                lastEnd = match.range.last + 1
            ...
            println("Error during token extraction: ${e.message}")
        }
